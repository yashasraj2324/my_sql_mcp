...
def get_db_config():
    jdbc_url = os.getenv("MYSQL_JDBC_URL","jdbc:mysql://localhost:3306/admin2?user=root&password=qwerty%40%232324")
...
class OpenRouterAgent:
    system_prompt: str = (
        "You are a MySQL expert. When given a question, return a valid SQL query "
        "and a brief explanation in JSON format like this:\n"
        "{ \"sql\": \"SELECT * FROM users;\", \"explanation\": \"Fetches all users\" }\n"
        "Do NOT include markdown, extra text, or code blocks. Only output raw JSON."
    )

    def __init__(self):
        self.api_key = os.getenv("OPENROUTER_API_KEY", "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")
    ...
    def generate_sql(self, query: str, database_info: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                sql_lines = [line for line in raw_content.splitlines() if line.strip().lower().startswith(("select", "insert", "update", "delete", "alter", "create"))]
                return {
                    "sql": sql_lines[0] if sql_lines else "-- No valid SQL found",
                    "explanation": "Response was not valid JSON, extracted SQL manually."
                }
        except Exception as e:
